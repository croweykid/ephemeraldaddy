...
If someone imports inside a locked-down environment (no pip / no network), it’ll blow up on import. That's because of the structure of the core/deps.py dependency autoinstaller. This is something I'm working to fix. I want to bundle all dependencies offline to prevent any future update-related external compatibility issues that destabilize the app.
