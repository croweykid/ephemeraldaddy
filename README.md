...
**Windows source installs should use 64-bit CPython 3.11.** The Swiss Ephemeris dependency (`pyswisseph`, imported as `swisseph`) publishes Windows wheels for Python 3.11. Newer Python versions (for example Python 3.14 from the Microsoft Store/`pythoncore-3.14`) can make `pip` fall back to compiling from source and fail with `Microsoft Visual C++ 14.0 or greater is required`. The easiest fix is to install Python 3.11 and recreate the venv:
...
If `py -3.11` is not found, install the Windows x86-64 Python 3.11 release from python.org, then reopen PowerShell.
