...
| 4 — core workflows | **Early/partial** | `ChartEditSession` and a callback-based `ChartEditorController` exist and are used incrementally. The pure `ChartRecalculationPolicy` now owns saved-chart change classification behind temporary window delegates, but the coordinator is not implemented. `DatabaseSelectionModel` and `DatabaseSelectionController` now own ordered logical UID selection, the navigation anchor, filtered-view merging, reconciliation, and one-step deselection restoration; Qt item mapping and persistence-boundary conversion remain in the window adapter. The session does not yet own the full lifecycle promised in section 6.1. The canonical Database Search query/evaluator/controller and non-Qt web-profile service modules do not yet exist. |
