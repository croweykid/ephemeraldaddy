...
def test_populate_list_skips_per_row_filter_engine_when_no_filters_are_active():
    ...
    assert (
        "has_active_chart_filters = self._has_active_chart_filters()" in populate_method
    )
    assert "if has_active_chart_filters:" in populate_method
    assert (
        "matches_filters = self._chart_matches_filters(\n                            chart_row,"
        in populate_method
    )
    assert "self._chart_uid_by_local_row_id" not in _method_source(
