...
def test_traits_distribution_collection_default_is_not_time_bounded(monkeypatch):
    owner = _TraitsCacheOwner((("uid:one", "row"),))
    owner._traits_distribution_chart_likelihood_cache = {}
    owner._get_chart_for_filter = lambda chart_id: {"id": chart_id}
    owner._is_placeholder_chart = lambda _chart: False
...
def test_database_trait_averages_requests_full_norm_collection(monkeypatch):
    owner = _TraitsCacheOwner((("uid:one", "row"),))
    seen_time_budgets = []

    def fake_collect(chart_ids, trait_items=None, trait_signature=None, time_budget_seconds="sentinel"):
        seen_time_budgets.append(time_budget_seconds)
        return {
            "trait_names": ["Creative"],
            "totals": {"Creative": 0.75},
            "chart_count": 1,
            "partial": False,
        }

    owner._collect_traits_distribution_analytics = fake_collect
    owner._traits_distribution_signature = lambda trait_items: (("Creative", "#ffffff", "{}"),)
    monkeypatch.setattr(trait_predictions, "_database_chart_ids", lambda _owner: (1,))
    monkeypatch.setattr(trait_predictions, "_load_trait_norm_cache", lambda: {})
    monkeypatch.setattr(trait_predictions, "_save_trait_norm_cache", lambda entries: None)

    averages = trait_predictions._database_trait_averages(owner, [{"name": "Creative", "profile": {}}])

    assert seen_time_budgets == [None]
    assert averages == {"Creative": 75.0}
