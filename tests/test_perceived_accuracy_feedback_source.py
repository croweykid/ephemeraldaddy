...
def test_property_thumbs_are_inside_the_chart_information_content_panel():
    builder = CHART_VIEW_SOURCE.split("chart_information_panel = QFrame()", 1)[1].split(
        "def build_chart_view_middle_header_controls", 1
    )[0]
    assert "parent=chart_information_panel" in builder
    assert "chart_information_layout.addWidget(owner.chart_info_output, 1)" in builder
    assert "chart_information_layout.addWidget(" in builder
    assert "owner.chart_information_accuracy_control" in builder
    header = CHART_VIEW_SOURCE.split("chart_info_header = QWidget()", 1)[1].split(
        "chart_information_panel = QFrame()", 1
    )[0]
    assert "chart_information_accuracy_control" not in header
