...
def test_database_analytics_display_names_use_uid_keyed_rows() -> None:
    source = Path("ephemeraldaddy/gui/features/charts/database_analytics.py").read_text(
        encoding="utf-8"
    )

    assert "def _display_name_for_chart_uid" in source
    assert "self._active_chart_rows_by_uid.get(normalized_uid)" in source
    assert "_display_name_for_chart_id" not in source
