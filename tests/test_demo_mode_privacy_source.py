...
def test_demo_mode_restore_uses_saved_visibility_and_all_private_active_tabs():
    app = (REPO_ROOT / "ephemeraldaddy/gui/app.py").read_text()

    assert 'def _restore_demo_mode_configured_visibility(self) -> None:' in app
    assert 'predictability_visible = bool(self._visibility.get("chart_view.predictability"))' in app
    assert 'section.setVisible(predictability_visible)' in app
