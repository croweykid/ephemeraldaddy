...
def test_gate_line_plugin_text_includes_exaltation_body_text() -> None:
    from ephemeraldaddy.analysis import human_design_plugins as plugins
    ...
    text = (
        ...
        "Saturn exalted.\n"
        "The inner strength to focus and realize. A natural gift for unlearnable knowing.\n"
        "Mars in detriment.\n"
        "Genius as madness. The delusion that knowledge is power."
    )

    assert plugins._trim_line_fixing_text(text, "exaltation", "Saturn") == (
        "Genius.\n"
        "Unconscious and unlearnable alignment of stimulus and response. The natural.\n"
        "Saturn exalted.\n"
        "The inner strength to focus and realize. A natural gift for unlearnable knowing."
...
def test_gate_line_plugin_text_includes_detriment_body_text() -> None:
    from ephemeraldaddy.analysis import human_design_plugins as plugins
    ...
    text = (
        ...
        "Venus exalted.\n"
        "The importance of aesthetics whether inborn or acquired.\n"
        "Mars in detriment.\n"
        "The assertion of ego in spite of wisdom. The urge for action that will ignore the wisdom of the Higher Self."
    )

    assert plugins._trim_line_fixing_text(text, "detriment", "Mars") == (
        "Intuition\n"
        "Sensitivity to disharmony and atrophy.\n"
        "Mars in detriment.\n"
        "The assertion of ego in spite of wisdom. The urge for action that will ignore the wisdom of the Higher Self."
...
def test_gate_line_plugin_text_handles_the_earth_detriment() -> None:
    from ephemeraldaddy.analysis import human_design_plugins as plugins

    text = (
        "Intelligent application.\n"
        "Mercury exalted.\n"
        "Reasoned management of resources.\n"
        "The Earth in detriment.\n"
        "The inability to share responsibility or recognize the abilities of others."
    )

    trimmed = plugins._trim_line_fixing_text(text, "detriment", "Earth")

    assert "The Earth in detriment." in trimmed
    assert "The inability to share responsibility" in trimmed
    assert "Reasoned management" not in trimmed
