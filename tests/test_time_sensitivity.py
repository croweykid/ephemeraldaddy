...
def test_compute_time_sensitivity_includes_off_interval_baseline_sample(monkeypatch):
    from datetime import datetime
    from types import SimpleNamespace

    from ephemeraldaddy.analysis import time_sensitivity as module
    from ephemeraldaddy.analysis.time_sensitivity import TimeSensitivityConfig, compute_time_sensitivity

    chart = SimpleNamespace(dt=datetime(2000, 1, 1, 8, 17), lat=0.0, lon=0.0, name="Example", birthtime_unknown=False)

    def fake_variant(_source, hour, minute):
        return SimpleNamespace(hour=hour, minute=minute)

    def fake_numeric(variant):
        value = 100.0 if (variant.hour, variant.minute) == (8, 17) else 10.0
        return {group: {"example": value} for group in module.NUMERIC_GROUPS}

    monkeypatch.setattr(module, "_variant_chart", fake_variant)
    monkeypatch.setattr(module, "_numeric_snapshot", fake_numeric)
    monkeypatch.setattr(module, "_categorical_snapshot", lambda variant: {})
    monkeypatch.setattr(module, "_hd_snapshot", lambda variant: {"gates": [], "lines": [], "channels": [], "type": "", "profile": ""})

    result = compute_time_sensitivity(chart, TimeSensitivityConfig(interval_minutes=720, include_day_end=False))
    payload = result.numeric_ranges["dominant_planet_weights"]["example"]

    assert result.baseline_time == "08:17"
    assert result.sample_count == 3
    assert payload["max"] == 100.0
    assert payload["peak_times"] == ["08:17"]
