...
def test_scores_human_design_and_bazi_metadata_categories():
    ...
    assert scores["match"] == 0.9610389610389609
...
def test_scores_bazi_signs_from_stored_pillars():
    ...
    assert scores["bazi"] == 0.3999999999999999


def test_criteria_weights_normalize_across_positive_items_without_category_weights():
    chart = SimpleNamespace(
    ...
    predictors = {
        "CHA": {
            "profiles": {"4/1": 3},
            "positions": {"Sun in Leo": 96, "Moon in Cancer": 1},
        }
    }

    scores = _score(predictors, chart)

    assert scores["CHA"] == 1.0
