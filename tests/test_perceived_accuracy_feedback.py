...
def test_thumbs_map_toggle_replace_and_clear_states(tmp_path):
    ...
    assert "#46c36f" in control.positive_button.styleSheet()
    assert "transparent" in control.negative_button.styleSheet()
    control.negative_button.click()
    assert control.state is False
    assert control.negative_button.isChecked()
    assert "#46c36f" in control.negative_button.styleSheet()
    assert "transparent" in control.positive_button.styleSheet()
    control.negative_button.click()
    assert control.state is None
