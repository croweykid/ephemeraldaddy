...
from ephemeraldaddy.gui.style import CHART_DATA_DIVIDER, SEPARATOR_STYLE, format_chart_header
        ...
        if use_houses:
            ...
            for index, column in enumerate(columns):
                ...
                if index < len(columns) - 1:
                    line_cursor += len(SEPARATOR_STYLE["text"])
            line = SEPARATOR_STYLE["text"].join(columns)
            entry_list = [
                {
                    "kind": "planet_keyword",
                    "body": body,
...
def _transit_tooltip_span(
    spans: dict[int, list[dict[str, object]]],
    line_number: int,
    line: str,
    token: str,
    tooltip: str,
    *,
    ...
    span_start = line.find(token) if start is None else start
    if span_start < 0:
        return
    spans.setdefault(line_number, []).append(
        {
...
def _zodiac_glyph(sign_name: str | None) -> str:
    if not sign_name:
        return "?"
    sign_glyphs = dict(zip(ZODIAC_NAMES, ZODIAC_SIGNS, strict=False))
    return sign_glyphs.get(sign_name, sign_name)


def _body_glyph_only(body: str) -> str:
    display_body = _display_body_name(body)
    return PLANET_GLYPHS.get(body) or PLANET_GLYPHS.get(display_body) or display_body


def _aspect_glyph_only(aspect_type: str) -> str:
    aspect_key = str(aspect_type).replace(" ", "_").lower()
    return ASPECT_GLYPHS.get(aspect_key, str(aspect_type))
...
def format_compact_transit_chart_text(
    ...
    *,
    ...
    lines.append(SEPARATOR_STYLE["text"].join(header_columns))
    lines.append("")

    required_chart_info_bodies = ["Chiron", "Ceres", "Pallas", "Juno", "Vesta"]
    missing_required = [body for body in required_chart_info_bodies if body not in chart.positions]
    if missing_required:
        try:
            refreshed_positions = planetary_positions(chart.dt, chart.lat, chart.lon)
        except Exception:
    ...
    if not getattr(chart, "retrogrades", None):
        try:
            ...
            chart.retrogrades = {}
    retrogrades = dict(getattr(chart, "retrogrades", {}) or {})
    ordered_bodies = [body for body in PLANET_ORDER if body in chart.positions or body in required_chart_info_bodies]
    ordered_bodies.extend(sorted(set(chart.positions).difference(ordered_bodies)))
    personality_gate_lines = _personality_gate_line_map(chart)

    for body in ordered_bodies:
        glyph = _body_glyph_only(body)
        lon = chart.positions.get(body)
        if lon is None or (not use_houses and body in angular_bodies):
            line = f"{_pad_display_column(glyph, body_width)}  Unknown"
            line_number = len(lines)
            _transit_tooltip_span(tooltip_spans, line_number, line, glyph, _display_body_name(body))
            lines.append(line)
            continue
        sign_label = sign_for_longitude(lon)
        ...
        _transit_tooltip_span(tooltip_spans, line_number, line, glyph, _display_body_name(body), start=body_start)
        _transit_tooltip_span(tooltip_spans, line_number, line, sign_glyph, sign_label, start=sign_start)
        lines.append(line)

    lines.append("")
    if use_houses and houses:
        lines.extend([CHART_DATA_DIVIDER, "HOUSES", CHART_DATA_DIVIDER])
        cusps = [None] * 12
        if isinstance(houses, (list, tuple)):
            for i, v in enumerate(houses[:12]):
                cusps[i] = v
            ...
            for k, v in houses.items():
                idx = int(k) - 1 if isinstance(k, int) else None
                if idx is not None and 0 <= idx < 12:
                    cusps[idx] = v
        for i, v in enumerate(cusps):
            if isinstance(v, (int, float)):
                sign_label = sign_for_longitude(float(v))
                sign_glyph = _zodiac_glyph(sign_label)
                line = f"{i + 1:>2}: {sign_glyph:<3} {_degree_in_sign_text(float(v))}"
                line_number = len(lines)
                _transit_tooltip_span(tooltip_spans, line_number, line, sign_glyph, sign_label)
                lines.append(line)
            else:
                lines.append(f"{i + 1:<2}: -")
        lines.append("")
        ...
        lines.extend(
            [CHART_DATA_DIVIDER, "HOUSES", CHART_DATA_DIVIDER, "Unknown (birth time unknown)", ""]
        )
    ...
    if filtered_aspects:
        ...
        dominant_planet_weights = getattr(
            ...
            None,
        ) or calculate_dominant_planet_weights(chart)
        sorted_aspects = sort_natal_aspects(
            filtered_aspects,
            "Priority",
            planet_weights=dominant_planet_weights,
        ...
        for asp in sorted_aspects:
            ...
            p2 = asp["p2"]
            atype = str(asp["type"])
            p1_sign = sign_for_longitude(positions[p1]) if p1 in positions else None
            p2_sign = sign_for_longitude(positions[p2]) if p2 in positions else None
            p1_label = f"{_body_glyph_only(p1)}{_zodiac_glyph(p1_sign)}"
            p2_label = f"{_body_glyph_only(p2)}{_zodiac_glyph(p2_sign)}"
            aspect_glyph = _aspect_glyph_only(atype)
            line = (
                f"{_pad_display_column(p1_label, endpoint_width)}" + SEPARATOR_STYLE["text"]
                + f"{_pad_display_column(aspect_glyph, aspect_width)}" + SEPARATOR_STYLE["text"]
                + f"{_pad_display_column(p2_label, endpoint_width)}" + SEPARATOR_STYLE["text"]
            ...
            aspect_start = line.find(aspect_glyph, p1_body_start + len(p1_label))
            p2_body_start = line.find(_body_glyph_only(p2), aspect_start + len(aspect_glyph))
            p2_sign_start = p2_body_start + len(_body_glyph_only(p2)) if p2_sign else -1
            _transit_tooltip_span(
                tooltip_spans,
                line_number,
                line,
                _body_glyph_only(p1),
            ...
            if p2_sign:
                _transit_tooltip_span(
                    ...
                    line_number,
                    line,
                    _zodiac_glyph(p2_sign),
                    p2_sign,
                    start=p2_sign_start,
                )
            lines.append(line)

    return "\n".join(lines), tooltip_spans
