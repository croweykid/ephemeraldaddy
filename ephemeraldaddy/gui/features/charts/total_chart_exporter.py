...
def _format_similar_chart_rows(rows: list[dict[str, Any]], *, markdown: bool) -> list[str]:
    if markdown:
        ...
        for row in rows:
            ...
            lines.append(
                f"| {row.get('rank', '')} | {row.get('display_chart_id') or ''} | {chart_name} | "
...
def build_total_chart_similar_charts_section(
    *,
    ...
    if markdown:
        lines = ["# Astro Twin Calculator", "", "## 25 Most Similar", ""]
