...
from ephemeraldaddy.analysis.human_design_reference import HD_COLORS, HD_LINE_COLORS, HD_TONES, LINE_NICKNAMES
...
def build_human_design_analytics_panel(
    *,
    ...
    color_labels = [
        f"C{value} ({str(color_meta_by_value.get(value, {}).get('name', 'unknown'))})"
    ...
    color_bars = hd_color_chart_ax.bar(color_labels, color_values, color=color_bar_colors, edgecolor="#E0E0E0", linewidth=0.5, alpha=0.95, picker=True)
    for bar, color_value in zip(color_bars, sorted(color_counts)):
        bar.set_gid(f"color:{color_value}")
