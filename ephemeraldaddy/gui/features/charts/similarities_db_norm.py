...
def similarity_delta_rgb(
    ...
    toward red. Saturation is based on standard-error distance when the row has
    a usable sample size, so a raw ten-percentage-point delta only looks fully
    saturated when it is also large relative to the row's expected variation.
    """
