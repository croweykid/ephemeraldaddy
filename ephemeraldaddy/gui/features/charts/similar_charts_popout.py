...
_PERCEIVED_ACCURACY_INPUT_BASE_STYLE = "font-size: 10px; padding: 1px;"
_PERCEIVED_ACCURACY_CHECKBOX_BASE_STYLE = "QCheckBox { font-size: 8pt; color: #f5f5f5; spacing: 3px; }"
_PERCEIVED_ACCURACY_RECORDED_INPUT_STYLE = (
    _PERCEIVED_ACCURACY_INPUT_BASE_STYLE
    + " border: 1px solid #35d07f; background-color: rgba(53, 208, 127, 0.24); color: #f5fff8;"
)
_PERCEIVED_ACCURACY_RECORDED_CHECKBOX_STYLE = (
    "QCheckBox { font-size: 8pt; color: #7CFC98; spacing: 3px; }"
    "QCheckBox::indicator { border: 1px solid #35d07f; background-color: rgba(53, 208, 127, 0.24); }"
    "QCheckBox::indicator:checked { background-color: #35d07f; }"
)
_PERCEIVED_ACCURACY_FAILED_INPUT_STYLE = (
    _PERCEIVED_ACCURACY_INPUT_BASE_STYLE
    + " border: 1px solid #d9534f; background-color: rgba(217, 83, 79, 0.18);"
)
_PERCEIVED_ACCURACY_FAILED_CHECKBOX_STYLE = (
    "QCheckBox { font-size: 8pt; color: #ffb3b3; spacing: 3px; }"
...
def _set_perceived_accuracy_recorded_style(
    line_edit: QLineEdit,
    ...
    *,
    ...
    if failed:
        line_edit.setStyleSheet(_PERCEIVED_ACCURACY_FAILED_INPUT_STYLE)
        na_checkbox.setStyleSheet(_PERCEIVED_ACCURACY_FAILED_CHECKBOX_STYLE)
        line_edit.setToolTip("Perceived accuracy was not recorded. Try editing this value again.")
        na_checkbox.setToolTip("No opinion / not enough familiarity was not recorded. Try toggling again.")
        return
    if recorded:
        line_edit.setStyleSheet(_PERCEIVED_ACCURACY_RECORDED_INPUT_STYLE)
        na_checkbox.setStyleSheet(_PERCEIVED_ACCURACY_RECORDED_CHECKBOX_STYLE)
        line_edit.setToolTip("Perceived accuracy for this match (0-100). Green means this value has been recorded.")
        na_checkbox.setToolTip("No opinion / not enough familiarity; green means this value has been recorded.")
        return
    line_edit.setStyleSheet(_PERCEIVED_ACCURACY_INPUT_BASE_STYLE)
    na_checkbox.setStyleSheet(_PERCEIVED_ACCURACY_CHECKBOX_BASE_STYLE)
    line_edit.setToolTip("Perceived accuracy for this match (0-100).")
    na_checkbox.setToolTip("No opinion / not enough familiarity; ignore this data point.")


def _chart_tags_for_generated_bio(chart: Any | None) -> list[str]:
    if chart is None:
        return []
    raw_tags = getattr(chart, "tags", None)
    if raw_tags is None:
...
def build_similar_chart_biography_text(*, compared_chart: Any | None) -> str:
    if compared_chart is None:
        return ""
    ...
    if not from_value:
        from_value = str(getattr(compared_chart, "from", "") or "").strip()
    if not from_value and isinstance(chart_metadata, Mapping):
        from_value = str(chart_metadata.get("from") or chart_metadata.get("from_whence") or "").strip()
    if from_value:
        generated_lines.append(f"from: {from_value}")

    alias_value = str(getattr(compared_chart, "alias", "") or "").strip()
    if not alias_value and isinstance(chart_metadata, Mapping):
        alias_value = str(chart_metadata.get("alias") or "").strip()
    if alias_value:
        generated_lines.append(f"aka: {alias_value}")
...
def _sentiment_estimate_html(average: float, median: float) -> str:
    ...
    avg_phrase = f"{html.escape(avg_emoji)} <span style='color:{avg_color}'>{html.escape(avg_label)}</span>"
...
_SIMILARITY_LABEL_HIGHLIGHT_PREFIXES: tuple[str, ...] = (
    ...
    "Top-3 overlap:",
    "Exact dominance mismatch:",
    "Sign dominance mismatch:",
    "Top sign differences:",
    "House dominance mismatch:",
    "Top house differences:",
    "Planet/body dominance mismatch:",
    "Top body differences:",
    "Top body-weighted placement-profile placement profiles",
    "Top nakshatra dominance profiles",
    "Defined center sets",
    # "",
    # "",
    # "",
...
def _distribution_summary(
    ...
    *,
    ...
    for body in CORE_BODIES:
        ...
        if subject_element and compared_element:
            element_possible += weight
            if subject_element == compared_element:
                element_points += weight
                element_matches.append(f"{body} {subject_element} ({weight:.2f} pts)")
        if subject_mode and compared_mode:
            mode_possible += weight
            if subject_mode == compared_mode:
                mode_points += weight
                mode_matches.append(f"{body} {subject_mode} ({weight:.2f} pts)")

    element_percent = (element_points / element_possible * 100.0) if element_possible > 0.0 else 0.0
    mode_percent = (mode_points / mode_possible * 100.0) if mode_possible > 0.0 else 0.0
    lines: list[str] = []
    if element_possible > 0.0:
        lines.append(
            f"Elemental body-match score: {element_percent:.1f}% "
            f"({element_points:.2f}/{element_possible:.2f} weighted points)."
        )
    if element_matches:
        lines.append(
            "Elemental matches by body: " + ", ".join(element_matches)
        )
    else:
        lines.append("Elemental matches by body: none.")
    if mode_possible > 0.0:
        lines.append(
            f"Modal body-match score: {mode_percent:.1f}% "
            f"({mode_points:.2f}/{mode_possible:.2f} weighted points)."
        )
    if mode_matches:
        lines.append(
            "Modal matches by body: " + ", ".join(mode_matches)
        )
    else:
        lines.append("Modal matches by body: none.")
    return lines
    ...
    for body in CORE_BODIES:
        ...
        if subject_element and compared_element and subject_element != compared_element:
            element_mismatches.append(
                f"{body}: {subject_element} ({subject_sign}) vs {compared_element} ({compared_sign}) [{weight:.2f} pts]"
            )
        if subject_mode and compared_mode and subject_mode != compared_mode:
            mode_mismatches.append(
                f"{body}: {subject_mode} ({subject_sign}) vs {compared_mode} ({compared_sign}) [{weight:.2f} pts]"
            )
    differences: list[str] = []
    if element_mismatches:
        differences.append("Elemental body mismatches: " + "; ".join(element_mismatches))
    if mode_mismatches:
        differences.append("Modal body mismatches: " + "; ".join(mode_mismatches))
    return differences
    ...
    if subject_chart is not None and compared_chart is not None:
        ...
        if analysis_mode == "dissimilarities":
            ...
            if "aspect" in component_weight_percents:
                ...
                lines.append("")
            if "distribution" in component_weight_percents:
                distribution_lines = _distribution_summary(
                    subject_chart,
                    compared_chart,
                    placement_weighting_mode=active_placement_mode,
                )
...
def render_similar_match_blocks(
    *,
    ...
    similarity_standard_deviation: float | None = None,
    start_rank: int = 1,
    expanded_why_targets: set[str] | None = None,
) -> str:
    if not matches:
    ...
    for rank, match in enumerate(matches, start=first_rank):
        ...
        why_target = make_similar_why_target(
            info_link_prefix=info_link_prefix,
...
def build_predictions_panel_content(
    *,
    ...
    positive_median_numeric = float(statistics.median(positive_values))
    positive_avg_numeric = float(statistics.fmean(positive_values))
    negative_median_numeric = float(statistics.median(negative_values))
    negative_avg_numeric = float(statistics.fmean(negative_values))

    positive_median = _fmt(positive_median_numeric)
    positive_avg = _fmt(positive_avg_numeric)
    negative_median = _fmt(negative_median_numeric)
    negative_avg = _fmt(negative_avg_numeric)
    ...
    *,
    ...
    def _panel(title: str, matches: list[Any], panel_key: str) -> QWidget:
        ...
        if not matches:
            ...
            def _render_plain_matches(*, label: QLabel = result_label) -> None:
                current_count = int(label.property("visible_count") or visible_count)
                label.setText(
                    render_similar_match_blocks(
                        matches=panel_all_matches[:current_count],
            ...
            def _handle_plain_link(target: str, *, label: QLabel = result_label) -> None:
                ...
                if is_similar_why_target(normalized_target):
                    ...
                    _render_plain_matches(label=label)
                    return
                on_link_activated(dialog, target)

            result_label.linkActivated.connect(_handle_plain_link)
            result_label.setStyleSheet(output_style)
            result_label.setProperty("visible_count", visible_count)
            _render_plain_matches()
            content_layout.addWidget(result_label)
            if len(panel_all_matches) > visible_count:
                ...
                def _show_more_plain(
                    ...
                    *,
                    ...
                    label.setProperty("visible_count", next_count)
                    _render_plain_matches(label=label)
                ...
                if na_checkbox.isChecked():
                    if line_edit.text().strip():
                        ...
                        if response != QMessageBox.Yes:
                            ...
                            _set_perceived_accuracy_recorded_style(line_edit, na_checkbox, recorded=False)
                            return
                        line_edit.clear()
            ...
            for rank, match in enumerate(panel_all_matches, start=1):
                ...
                na_checkbox.setMinimumWidth(34)
                _set_perceived_accuracy_recorded_style(accuracy_input, na_checkbox, recorded=False)
                ...
                if raw_score is not None:
                    try:
                        ...
                        pass
                has_recorded_state = isinstance(state, Mapping) and bool(state)
