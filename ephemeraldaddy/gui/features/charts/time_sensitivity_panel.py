...
def _variability_scale_label(percent_delta_spread: float) -> str:
    """Return the table-facing variability label without alarmist extremes."""
    spread = abs(float(percent_delta_spread))
    return "medium" if spread < 35.0 else "high"
...
def _human_design_html(result: TimeSensitivityResult) -> str:
    ...
    centers_summary = hd.get("defined_centers", {})
    possible_centers = (
        centers_summary.get("sometimes", [])
        if isinstance(centers_summary, dict)
        else []
    )
    center_bits = [
        _hd_property_anchor("defined_center", str(center))
        for center in possible_centers
        if str(center)
    ]
    hd_items.append("Possible Defined Centers: " + (", ".join(center_bits) or "none"))
...
def human_design_time_range_text(
    ...
    """Return approximate sampled Time Sensitivity spans for an HD gate or gate/line."""
    if result is None:
        return ""
    summary_key = "lines" if line is not None else "gates"
    item_key = f"{int(gate)}.{int(line)}" if line is not None else str(int(gate))
    summary = result.human_design.get(summary_key, {})
    if not isinstance(summary, dict):
        return ""
    spans = (summary.get("presence_spans") or {}).get(item_key)
    count = (summary.get("presence_counts") or {}).get(item_key)
    sample_count = int(summary.get("sample_count") or result.sample_count or 0)
    label = f"Gate {item_key}" if line is not None else f"Gate {int(gate)}"
    if spans:
        span_text = "; ".join(str(span) for span in spans)
    elif count == sample_count and sample_count:
        span_text = "all sampled times"
    elif count:
        span_text = "sampled times unavailable"
    else:
        return ""
    return f"Approx. Time Sensitivity range for {label}: {span_text} (rounded to sampled 30-minute blocks)."
