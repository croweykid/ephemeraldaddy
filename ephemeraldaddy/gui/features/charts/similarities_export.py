...
def similarities_export_samples_tuple(samples: int = 0) -> tuple[int, int]:
    """Return the canonical trait export sample tuple: (positive, anti)."""
...
def empty_similarities_json_profile(selection_name: str, samples: int = 0) -> OrderedDict:
    ...
    profile = OrderedDict([("name", selection_name), ("samples", similarities_export_samples_tuple(samples))])
...
def build_similarities_json_export_payload(
    ...
    if _has_dissimilarity_json_owner(export_sections):
        ...
        bundle = OrderedDict([("name", selection_name), ("samples", similarities_export_samples_tuple(sample_size))])
