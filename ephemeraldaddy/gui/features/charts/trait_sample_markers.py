...
def _marked_name(
    ...
    unknown_marker_names: set[str] | None = None,
) -> str:
    text = str(name or "")
    if text in (unknown_marker_names or set()):
        return f"{UNKNOWN_SAMPLE_TRAIT_PREFIX}{text}"
...
def _profile_has_sample_uids(profile: Mapping[str, Any]) -> bool:
    """Return whether canonical source-sample provenance is present and non-empty."""
    if "sample_uids" not in profile:
        return False
    raw_uids = profile.get("sample_uids")
    if not isinstance(raw_uids, (list, tuple, set, frozenset)):
        return False
    return any(str(value or "").strip() for value in raw_uids)


def _unknown_sample_trait_names(traits: list[dict[str, Any]]) -> set[str]:
    """Return Traits whose canonical source-sample provenance is unavailable."""
    return {
        name
        for trait in traits
        if (name := str(trait.get("name", "") or "").strip())
        and not _profile_has_sample_uids(_trait_profile(trait))
    }
...
def install_trait_sample_markers(core: ModuleType) -> None:
    ...
    applied only to presentation copies immediately before rendering. Traits
    without a non-empty canonical ``sample_uids`` property use ❓ because their
    source-sample membership cannot be distinguished from a true prediction.
    """
    ...
    def rows_from_metadata_with_sample_markers(
        ...
        unknown_marker_names = _marker_names(core, _UNKNOWN_MARKER_NAMES_ATTR)
        if not marker_names and not anti_marker_names and not unknown_marker_names:
            return rows
        marked_rows: list[dict[str, Any]] = []
        for row in rows:
            marked = dict(row)
    ...
    def rank_row_with_sample_marker(
        ...
        *,
        ...
        marked = _marked_name(
            ...
            _marker_names(core, _UNKNOWN_MARKER_NAMES_ATTR),
        )
        if marked == name:
            return rendered
    ...
    def apply_metadata_with_sample_markers(
        ...
        *,
        ...
        try:
            ...
            if previous_unknown is _MISSING:
                try:
                    delattr(core, _UNKNOWN_MARKER_NAMES_ATTR)
                except AttributeError:
                    pass
            else:
                setattr(core, _UNKNOWN_MARKER_NAMES_ATTR, previous_unknown)
