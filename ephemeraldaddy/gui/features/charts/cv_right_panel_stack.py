...
def _resolve_chart_right_panel_key(owner: object, panel_key: str) -> str:
    ...
    if normalized == "subjective_notes" and bool(getattr(owner, "_demo_mode_enabled", False)):
...
def sync_chart_right_panel_placeholder_state(owner: object, chart: object | None) -> None:
    ...
    if bool(getattr(owner, "_demo_mode_enabled", False)):
        subjective_button = getattr(owner, "subjective_notes_panel_button", None)
        subjective_scroll = getattr(owner, "subjective_notes_panel_scroll", None)
        if subjective_button is not None:
            subjective_button.setVisible(False)
        if subjective_scroll is not None:
            subjective_scroll.setVisible(False)
