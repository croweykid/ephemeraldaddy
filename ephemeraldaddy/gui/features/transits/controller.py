...
class TransitPanelController:
    ...
    def __init__(
        self, host: Any, *, get_popout_window_icon_path: Any | None = None
    ) -> None:
        self.host = host
    ...
    def _install_legacy_state(self) -> None:
        ...
        self.window_cache = TransitWindowCache()
        h._transit_window_result_cache = self.window_cache.results
        h._transit_window_metrics = self.window_cache.metrics
    ...
    def transit_window_cache_key(
        self,
        *,
        mode: str,
        hit_obj: Any,
        chart_dt: datetime.datetime,
        transit_location: tuple[float, float],
        mode_rules: dict[str, Any],
        scan_config: Any,
    ) -> tuple[object, ...]:
        return self.window_cache.build_key(
            mode=mode,
            hit_obj=hit_obj,
            chart_dt=chart_dt,
    ...
    def get_transit_window_cache(
        self, cache_key: tuple[object, ...]
    ) -> dict[str, object] | None:
        return self.window_cache.get(cache_key)

    def put_transit_window_cache(
        self, cache_key: tuple[object, ...], payload: dict[str, object]
    ) -> None:
        self.window_cache.put(cache_key, payload)

    def record_transit_window_inflight_dedupe(self) -> None:
        self.window_cache.record_inflight_dedupe()
    ...
    def build_panel(self) -> QWidget:
        ...
        h.transit_location_label.setStyleSheet(
        ...
        h.personal_transit_chart_input.returnPressed.connect(
        ...
        h.generate_personal_transit_button.clicked.connect(
            self.on_personal_transit_generate_clicked
        )
        ...
        h.todays_transits_updated_label.setStyleSheet(
            "font-size: 11px; color: #a5a5a5; padding: 0 2px 4px 2px;"
        )
        ...
        h.todays_transits_output.setSizePolicy(
    ...
    def apply_location(self, *, show_errors: bool = True) -> None:
        ...
        if parsed_lat is not None and parsed_lon is not None:
            if not (-90.0 <= parsed_lat <= 90.0 and -180.0 <= parsed_lon <= 180.0):
                ...
                return
            h._transit_lat = parsed_lat
            h._transit_lon = parsed_lon
            h._transit_location_label = f"{parsed_lat:.4f}, {parsed_lon:.4f}"
            h._transit_location_source = "manual"
            self.save_location_preference(raw_value)
            self.refresh_panel()
            return
        try:
            ...
            return
        h._transit_lat = float(lat)
        h._transit_lon = float(lon)
        h._transit_location_label = resolved_label
        h._transit_location_source = "manual"
        self.save_location_preference(raw_value)
        self.refresh_panel()
    ...
    def save_location_preference(self, raw_location: str) -> None:
        self.host._settings.setValue(
            "manage_charts/transit_last_location", raw_location.strip()
        )
    ...
    def refresh_panel(self) -> None:
        h = self.host
        if not hasattr(h, "todays_transits_chart_layout"):
            return
        h._clear_layout(h.todays_transits_chart_layout)
        h._transit_chart_canvases.clear()
        ...
        chart.birth_place = h._transit_location_label
        ...
        canvas.installEventFilter(h)
        chart_click_container.installEventFilter(h)
        h._transit_chart_canvases[canvas] = chart
        ...
        source_hint = (
            ...
            else " [Saved]" if h._transit_location_source == "manual" else ""
        )
        h.transit_location_label.setText(
            f"Location: {h._transit_location_label}{source_hint} | Lat/Lon: {h._transit_lat:.4f}, {h._transit_lon:.4f}"
        )
        if include_time:
            h.todays_transits_updated_label.setText(
                f"Selected local time: {local_now.strftime('%Y-%m-%d %H:%M %Z')}"
            )
        else:
            h.todays_transits_updated_label.setText(
                f"Selected date (time omitted): {local_now.strftime('%Y-%m-%d')}"
            )
    ...
    def on_use_time_toggled(self, use_time: bool) -> None:
        self.host.transit_time_input.setEnabled(use_time)
        self.refresh_panel()
    ...
    def matching_personal_transit_labels(self, raw: str) -> list[str]:
        labels = list(self.host._personal_transit_chart_lookup.keys())
        query = raw.strip().lower()
        return (
            labels
            if not query
            else [label for label in labels if query in label.lower()]
        )
    ...
    def on_personal_transit_completer_activated(self, label: str) -> None:
        ...
        if not selected_label:
            return
        h = self.host
        h.personal_transit_chart_input.setText(selected_label)
        h.personal_transit_chart_input.setCursorPosition(len(selected_label))
    ...
    def on_personal_transit_enter_pressed(self) -> None:
        ...
        h.personal_transit_chart_input.setText(first_match)
        h.personal_transit_chart_input.setCursorPosition(len(first_match))
        if len(matches) == 1:
            self.generate_personal_transit()
    ...
    def generate_personal_transit(self) -> None:
        ...
        try:
            ...
            place_label = getattr(h, "_transit_location_label", "Unknown")
            timestamp_label = (
            ...
            natal_targets = assign_houses(
                natal_normalized.bodies, natal_normalized.houses, layer="NATAL"
            )
            ...
            h._show_personal_transit_chart_popout(
                natal_chart,
                transit_chart,
                transit_in_natal,
                {
