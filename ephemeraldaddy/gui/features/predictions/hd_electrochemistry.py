...
HD_RESONANCE_SUBHEADER = (
    "Top 10 charts with the most shared Human Design gate-line activations, then shared gates."
)
...
def resolve_hd_resonance_gate_lines(
    chart: object | None,
) -> frozenset[tuple[int, int]]:
    """Return canonical HD activation pairs while preserving their gate identity."""
    if chart is None:
        return frozenset()
    try:
...
def hd_electrochemistry_render_token(owner: object, chart: object | None) -> tuple[object, ...]:
    ...
    return (
        ...
        tuple(sorted(resolve_hd_resonance_gate_lines(chart))),
...
def _format_hd_electrochemistry_matches(matches: tuple, warning_lines: tuple[str, ...]) -> str:
    ...
    if resonance_results:
        lines.append("Ranked by exact gate-line matches, then shared gates.")
    elif norms is None:
    ...
    for index, match in enumerate(matches, 1):
        ...
        if norms is not None and norms.sample_size:
            database_norms = (
                f"; database-wide {norms.percentile_for_score(electrochemistry_score):.0f}th "
                "percentile before profile bonus"
            )
        lines.append(
            f'{index}. <a href="{href}" style="color: #cdb7ff;">'
            f"{html.escape(display_name)}</a>{uncertainty_html} "
            f'<span style="color: #aaa;">(score {match.score}/{score_maximum}: '
...
def render_hd_electrochemistry_predictions(owner: object, chart: object | None) -> None:
    ...
    matches = ranker(
        ...
        source_gate_lines=resolve_hd_resonance_gate_lines(chart),
    )
    if not matches:
