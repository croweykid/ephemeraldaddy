...
from ephemeraldaddy.gui.style import perceived_accuracy_button_style
...
def property_target_from_entry(
    ...
    identity_fields = (
        ...
        "p1", "p2", "type", "decan", "sign1", "sign2", "house1", "house2",
    )
    identity = [
...
class PerceivedAccuracyThumbs(QWidget):
    ...
    def _render(self, state: bool | None) -> None:
        ...
        for button, selected in (
            (self.positive_button, state is True),
            (self.negative_button, state is False),
        ):
            button.setStyleSheet(perceived_accuracy_button_style(selected=selected))
