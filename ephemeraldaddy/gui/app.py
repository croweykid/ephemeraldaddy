...
class MainWindow(QMainWindow):
    ...
    def _selected_chart_id_from_manage_view(self) -> int | None:
        ...
        if manage_dialog is None or not hasattr(manage_dialog, "list_widget"):
            return None
        list_widget = manage_dialog.list_widget
        selected_items = list_widget.selectedItems()
        item = None
        if selected_items:
            # Get Single Chart rule:
            # When multiple charts are selected in Database View and a single-chart
            # action is requested, use the highest chart in current sort order.
            item = min(selected_items, key=list_widget.row)
        elif list_widget.currentItem() is not None:
