...
def _is_optional_module_enabled(owner: "QWidget", key: str) -> bool:
    ...
    if not callable(get_visibility):
        return False
    return bool(get_visibility(f"optional_modules.{key}"))


def _is_human_design_menu_enabled(owner: "QWidget") -> bool:
    return _is_optional_module_enabled(owner, "human_design")


def _is_event_predictor_menu_enabled(owner: "QWidget") -> bool:
    return _is_optional_module_enabled(owner, "event_predictor")
...
def configure_main_window_chrome(window: "QMainWindow") -> None:
    ...
    if _is_event_predictor_menu_enabled(window):
        _bind_menu_action(tools_menu, "Event Predictor", window, "on_open_event_predictor")
...
def configure_manage_dialog_chrome(dialog: "QWidget", layout: "QLayout") -> None:
    ...
    if _is_event_predictor_menu_enabled(dialog):
        _bind_menu_action(tools_menu, "Event Predictor", dialog, "_on_menu_open_event_predictor")
