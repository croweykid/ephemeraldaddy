...
def _normalize_event_planner_criteria(
    criteria: dict[str, dict[str, object]],
) -> dict[str, dict[str, set[str]]]:
    normalized: dict[str, dict[str, set[str]]] = {}
    zodiac_set = set(ZODIAC_NAMES)
    for body, body_criteria in criteria.items():
        if body not in RETCON_BODIES or not isinstance(body_criteria, dict):
            continue
        include = {str(sign) for sign in body_criteria.get("include", []) if str(sign) in zodiac_set}
        exclude = {str(sign) for sign in body_criteria.get("exclude", []) if str(sign) in zodiac_set}
        if include or exclude:
            normalized[body] = {"include": include, "exclude": exclude}
    return normalized


def _event_planner_search_windows(
    start_dt: dt.datetime,
    end_dt: dt.datetime,
    criteria: dict[str, dict[str, set[str]]],
    lat: float,
    lon: float,
    should_cancel_cb: Callable[[], bool] | None,
) -> list[tuple[dt.datetime, dt.datetime]]:
    required_slow_signs = {
        body: next(iter(body_criteria["include"]))
        for body, body_criteria in criteria.items()
        if body in SLOW_RETCON_BODIES and len(body_criteria["include"]) == 1
    }
    return _candidate_search_windows(start_dt, end_dt, required_slow_signs, lat, lon, should_cancel_cb)


def search_event_planner_candidates(
    criteria: dict[str, dict[str, object]],
    start_dt: dt.datetime,
    end_dt: dt.datetime,
    lat: float,
    ...
    *,
    ...
    """Search future electional astrology candidates matching include/exclude sign criteria."""
    if start_dt.tzinfo is None or end_dt.tzinfo is None:
        raise ValueError("search_event_planner_candidates expects timezone-aware datetimes")
    if end_dt < start_dt:
        raise ValueError("end_dt must be on or after start_dt")
    if step_minutes <= 0:
        raise ValueError("step_minutes must be positive")

    normalized = _normalize_event_planner_criteria(criteria)
    if not normalized:
        return []

    search_windows = _event_planner_search_windows(
        start_dt,
        end_dt,
        normalized,
        lat,
    ...
    for window_start, window_end in search_windows:
        ...
        while current <= window_end and len(results) < max_results:
            ...
            is_match = True
            matched_positions: dict[str, float] = {}
            matched_signs: dict[str, str] = {}
            for body, body_criteria in normalized.items():
                lon_value = positions.get(body)
                if lon_value is None:
                    is_match = False
                    break
                sign = zodiac_sign_for_longitude(lon_value)
                include = body_criteria["include"]
                exclude = body_criteria["exclude"]
                if include and sign not in include:
                    is_match = False
                    break
                if exclude and sign in exclude:
                    is_match = False
                    break
                matched_positions[body] = float(lon_value)
                matched_signs[body] = sign

            if is_match:
                match = {"datetime": current, "positions": matched_positions, "signs": matched_signs}
                results.append(match)
                if match_cb is not None:
                    match_cb(match)
