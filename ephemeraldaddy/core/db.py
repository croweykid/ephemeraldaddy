...
def _create_dnd_prediction_metadata_table(conn: sqlite3.Connection) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS chart_dnd_prediction_metadata (
...
def upsert_chart_dnd_prediction_metadata(chart_uid: str, payload: Mapping[str, Any]) -> None:
    ...
    try:
        with conn:
            ...
            conn.execute(
                """
                INSERT INTO chart_dnd_prediction_metadata (chart_uid, payload, updated_at)
                VALUES (?, ?, ?)
...
def _serialize_quotes(quotes: Optional[list[str]]) -> Optional[str]:
    return _serialize_string_list(quotes)


def parse_quotes(value: Optional[str]) -> list[str]:
...
def update_chart_mortality_by_uids(
    chart_uids: Iterable[str | None],
    *,
    is_deceased: bool,
) -> set[str]:
    """Persist mortality and its dependent unknown death-time fact atomically.

    Marking a chart deceased also marks its death time unknown, preserving the
    Batch Editor's established behavior and preventing the default noon widget
    value from becoming factual when no death time has been entered.
    Marking it living does not erase the previously recorded death-time state.
    """
    normalized_uids = sorted(
        {
            normalized_uid
    ...
    with _get_conn() as conn:
        ...
        for start in range(0, len(normalized_uids), 900):
            uid_chunk = normalized_uids[start:start + 900]
            placeholders = ", ".join("?" for _ in uid_chunk)
            changed_uids.update(
                str(row[0])
                for row in conn.execute(
            ...
            if is_deceased:
                conn.execute(
                    f"""
                    UPDATE charts
                    SET is_deceased = 1, deathtime_unknown = 1
                ...
                conn.execute(
                    f"UPDATE charts SET is_deceased = 0 "
                    f"WHERE chart_uid IN ({placeholders})",
                    uid_chunk,
                )
    return changed_uids
