...
def _warn_similarity_metadata_repaired(chart: Chart, repaired_fields: list[str], persisted: bool) -> None:
    ...
    warnings.warn(
        f"Repaired missing {', '.join(repaired_fields)} for "
        f"{_similarity_chart_label(chart)} ({persistence_note}).",
...
def build_body_dominance_explanation_bullets(
    ...
    natural_planet_bonus = 6.0 if house_num and NATURAL_HOUSE_PLANETS.get(house_num) == canonical_body else 0.0
