...
def _scan_times_with_baseline(
    interval_minutes: int,
    *,
    include_day_end: bool,
    baseline_hour: int,
    baseline_minute: int,
) -> list[tuple[int, int]]:
    times = scan_times(interval_minutes, include_day_end=include_day_end)
    baseline = (int(baseline_hour), int(baseline_minute))
    if baseline not in times:
        times.append(baseline)
        times.sort(key=lambda time: (time[0] * 60) + time[1])
    return times
...
def compute_time_sensitivity(chart: Any, config: TimeSensitivityConfig | None = None) -> TimeSensitivityResult:
    ...
    for hour, minute in _scan_times_with_baseline(
        cfg.interval_minutes,
        include_day_end=cfg.include_day_end,
